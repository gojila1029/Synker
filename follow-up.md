...
| P2-4 | Security | **Development credentials shared in chat history must be rotated.** Supabase JWT secret, database password, service role key, and anon key were transmitted in session transcript. Treat all four as compromised. | Supabase Dashboard (rotate keys), local `.env`, `.env.local` | open | ACTION REQUIRED: rotate all four credentials in Supabase Dashboard immediately. This cannot be done in code. Production audit 2026-08-04: /health now returns 503 on DB failure, CI install fixed (uv sync --all-groups), stale security.py docstring removed. Credentials still unrotated — the only remaining deploy blocker. |
