...
| P2-4 | Security | **Development credentials shared in chat history must be rotated.** Supabase JWT secret, database password, service role key, and anon key were transmitted in session transcript. Treat all four as compromised. | Supabase Dashboard (rotate keys), local `.env`, Railway env vars | open | MANUAL ACTION — cannot be automated without Supabase account access. Steps: (1) Supabase Dashboard > Project Settings > API > rotate JWT secret + copy anon/service-role keys. (2) Supabase Dashboard > Database > Settings > reset DB password. (3) Update backend/.env. (4) In Railway: set SUPABASE_JWT_SECRET, DATABASE_URL, and any other secrets. Repo pushed to GitHub — do NOT push .env after rotating. |
